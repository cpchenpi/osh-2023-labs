...
EDIT：异步部分应要求单独作为了一个项目`lab3_async`。与`lab3`相同，它也是一个普通的Cargo项目，有相同的编译运行方式。
...
这一项提供了多种选择，因为似乎前几年异步编程比较热门，我选择了异步I/O。在调研资料时，我发现Rust异步编程相关的内容似乎在网络上较少。所幸，Rust官方提供了[async-book](https://rust-lang.github.io/async-book)，它相当不完善，有大量待补全的内容，但足够用于异步的基础了解了。

Async book告诉我们，异步编程相比多线程模型的优点在于可以用少量线程（理论上可以使用单线程）处理大量并发任务，节省资源，它理论上性能并不一定比多线程强；另外，异步编程相对于非阻塞同步（I/O复用等）编程，其逻辑上更接近同步编程，易于开发。Rust不提供异步运行时，因此我们必须选择一个合适的库，这里我选择tokio。

抽象分析该问题。同样地，由于任务之间较强的并行性和任务内部的串行性，我们可以直接用`tokio::spawn`对每个请求独立的启动处理，而处理内部只简单地改为异步I/O。

对异步版本重复上面的测试，得到数据如下：

| 文件大小 | 并发数 | 轮数 | 总用时(s) | 可用性 | 吞吐量(MB/s) | 并发能力 | 最长传输时间 |
| :-----: | :---: | :-: | :------: | :---: | :---------: | :-----: | :-------: |
| 5KB     | 255   | 256 | 7.05     | 100%  | 42.23       | 250.69  | 0.17      |
| 100KB   | 255   | 216 | 12.77    | 100%  | 615.32      | 250.30  | 0.18      |
| 1MB     | 255   | 64  | 18.26    | 100%  | 1126.73     | 252.17  | 0.57      |
| 50MB    | 255   | 8   | 18.47    | 100%  | 1126.54     | 251.63  | 4.03      |
| 500MB   | 8     | 6   | 26.85    | 100%  | 991.73      |   7.97  | 7.64      |

可以看到，该异步版本的吞吐量普遍不如线程池，甚至普通单线程版本。这个问题可能是比较复杂且具体的，比如可能与运行时的具体调度方式等有较大的关系：tokio甚至直接称自己为"A runtime for writing reliable network applications without compromising speed"，不保证程序的速度。

当然，异步版本有一个明显的优势：并发能力相比之前的两个版本都有明显提升，最长传输时间在文件不大的时候也有明显提升。这可能与线程较少，更易于避免饥饿的调度有关。